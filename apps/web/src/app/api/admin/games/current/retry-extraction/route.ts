...
import { getCurrentAdminGameId, retryExtraction, WorkerUnavailableError } from '@/server/game/admin-lifecycle';
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const claim = await claimIdempotency({ actorScope: `admin:${adminNickname}`, operation: 'RETRY_EXTRACTION', key: request.headers.get('Idempotency-Key') ?? '', payload: {} });
    if (claim.kind === 'REPLAY') return ok({ status: 'RETRY' as const });

    const gameId = await getCurrentAdminGameId(getDb());
    if (!gameId) return apiError('NO_CURRENT_GAME', 409, true);
    await retryExtraction(gameId);
