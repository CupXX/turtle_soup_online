...
import { forceEndGame, GameLifecycleStateError, getCurrentAdminGameId } from '@/server/game/admin-lifecycle';
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    if (body.confirmation !== 'FORCE_END') throw new InputValidationError('confirmation is required');
    ...
    if (claim.kind === 'REPLAY' && claim.resultResourceId) {
      return ok({ status: 'ENDED' as const, endReason: 'FORCE_ENDED' as const });
    }

    const gameId = await getCurrentAdminGameId(getDb());
    if (!gameId) return apiError('NO_CURRENT_GAME', 409, true);
    const result = await forceEndGame(gameId);
    if (claim.kind === 'NEW') {
      await bindIdempotencyResult({ actorScope, operation, key: request.headers.get('Idempotency-Key') ?? '', resultResourceId: gameId, responseStatus: 200 });
    }
    return ok(result);
  } catch (error) {
    ...
    if (error instanceof GameLifecycleStateError) {
      return apiError(error.message === 'NO_CURRENT_GAME' ? 'NO_CURRENT_GAME' : 'GAME_NOT_ACTIVE', 409, true);
    }
