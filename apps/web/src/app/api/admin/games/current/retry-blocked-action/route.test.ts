...
vi.mock('@/server/game/admin-lifecycle', () => ({
  ...
  retryBlockedAction: mocks.retryBlockedAction,
  BlockedActionRetryError: class extends Error {},
...
function request() {
  ...
  return new Request('http://localhost:3000/api/admin/games/current/retry-blocked-action', {
    method: 'POST',
    headers: {
...
describe('POST /api/admin/games/current/retry-blocked-action', () => {
  it('retries the blocked head through the authenticated admin boundary', async () => {
    validEnv();
    ...
    mocks.retryBlockedAction.mockResolvedValue(undefined);

    const response = await POST(request());

    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({ data: { status: 'RETRY' } });
    expect(mocks.retryBlockedAction).toHaveBeenCalledWith(gameId);
