...
import { getCurrentAdminGameId, replacePreparation, WorkerUnavailableError } from '@/server/game/admin-lifecycle';
...
export async function PUT(request: Request): Promise<Response> {
  try {
    ...
    const claim = await claimIdempotency({ actorScope: `admin:${adminNickname}`, operation: 'REPLACE_PREPARATION', key: request.headers.get('Idempotency-Key') ?? '', payload: { puzzleSurface, fullSolution } });
    if (claim.kind === 'REPLAY') return ok({ status: 'WAITING' as const });

    const gameId = await getCurrentAdminGameId(getDb());
    if (!gameId) return apiError('NO_CURRENT_GAME', 409, true);
    await replacePreparation(gameId, { puzzleSurface, fullSolution });
